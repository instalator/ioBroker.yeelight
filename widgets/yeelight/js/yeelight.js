...
// this code can be placed directly in yeelight.html
vis.binds.yeelight = {
    version: "0.5.0",
    showVersion: function () {
        if (vis.binds.yeelight.version) {
            console.log('Version yeelight: ' + vis.binds.yeelight.version);
            vis.binds.yeelight.version = null;
        }
    },
	createWidget: function (widgetID, view, data, style) {
        ...
        if (!$div.length) {
            return setTimeout(function () {
                vis.binds.yeelight.createWidget(widgetID, view, data, style);
            }, 100);
        }
        ...
        if (data.oid) {
            vis.states.bind(data.oid + '.val', function (e, newVal, oldVal) {
                $div.find('.yeelight-value').html(newVal);
            });
        }
    }
};
	
vis.binds.yeelight.showVersion();
